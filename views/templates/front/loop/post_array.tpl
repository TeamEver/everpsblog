...
<article class="col-12 mb-4" id="everpsblog-{$item.id_ever_post|escape:'htmlall':'UTF-8'}">
    <div class="card h-100 shadow-sm border-0 everpsblog everpsblog-listing-card overflow-hidden">
        <div class="row g-0 h-100 align-items-stretch">
            ...
            <div class="mt-auto">
                <a href="{$link->getModuleLink('everpsblog', 'post', ['id_ever_post' => $item.id_ever_post , 'link_rewrite' => $item.link_rewrite])|escape:'htmlall':'UTF-8'}" class="btn btn-primary rounded-pill px-4 {$blogcolor|escape:'htmlall':'UTF-8'} fw-semibold" title="{$item.title|escape:'htmlall':'UTF-8'} {$shop.name|escape:htmlall:'UTF-8'}">{l s='Lire la suite' mod='everpsblog'}</a>
            </div>
        </div>
            </div>
        </div>
